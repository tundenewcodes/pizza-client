...
const BASE_URL="https://pizza-api-rgfi.onrender.com"
...
export const addPizza = (pizza) => async (dispatch) => {
  ...
  try {
    await axios.post(`${BASE_URL}/api/pizzas/addpizza`, { pizza });
...
export const updatePizza = (updatedPizza) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(`${BASE_URL}/api/pizzas/updatepizza`, {
      updatedPizza,
    });
...
export const deletePizza = (pizzaId) => async (dispatch) => {
  try {
    await axios.post(`${BASE_URL}/api/pizzas/deletepizza`, { pizzaId });
...
export const filterPizza = (searchkey, category) => async (dispatch) => {
  ...
  dispatch({ type: "GET_PIZZAS_REQUEST" });
  try {
    const res = await axios.get(`${BASE_URL}/api/pizzas/getAllPizzas`);
